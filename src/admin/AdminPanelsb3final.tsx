  ...
  const renderForm = () => (
    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md max-w-2xl mx-auto w-full">
      ...
      {activeMenu === 'remarketing' && (
        <div className="mt-8">
          <h3 className="text-lg font-semibold mb-4">Remarketing Cadastrados</h3>
          <ul className="space-y-4">
            {itemList.filter(item => item.type === 'remarketing').map((item, idx) => (
              <li key={idx} className="p-4 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
                <p className="text-sm mb-1">{item.text}</p>
                {item.date && <p className="text-xs text-gray-500">Data: {item.date}</p>}
                {item.image && <p className="text-xs text-gray-500">Imagem: {item.image}</p>}
              </li>
            ))}
            {itemList.filter(item => item.type === 'remarketing').length === 0 && (
              <p className="text-sm text-gray-500">Nenhum conteúdo de remarketing cadastrado ainda.</p>
            )}
          </ul>
        </div>
      )}
...
{activeMenu === 'posts' && (
        <div className="mt-8">
          <h3 className="text-lg font-semibold mb-4">Posts Cadastrados</h3>
          <ul className="space-y-4">
            {itemList.filter(item => item.type === 'posts').map((item, idx) => (
              <li key={idx} className="p-4 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
                <p className="text-sm mb-1">{item.text}</p>
                {item.date && <p className="text-xs text-gray-500">Data: {item.date}</p>}
                {item.image && <p className="text-xs text-gray-500">Imagem: {item.image}</p>}
            ...
            {itemList.filter(item => item.type === 'posts').length === 0 && (
              <p className="text-sm text-gray-500">Nenhum post cadastrado ainda.</p>
            )}
          </ul>
        </div>
      )}
