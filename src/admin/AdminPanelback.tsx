...
import { FileText, Mail, Settings as Cog, LogOut, BarChart2, AlertCircle, Users, Upload } from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose, isDarkMode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/.netlify/functions/post', { method: 'POST', body: formData });
      const data = await response.json();
