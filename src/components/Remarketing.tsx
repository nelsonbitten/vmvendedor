...
const Remarketing: React.FC = () => {
  ...
  const [remarketingDoDia, setRemarketingDoDia] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchRemarketing = async () => {
      try {
        ...
        // 🔎 Verifica se existe um remarketing com a data de hoje
        const hoje = new Date().toISOString().split('T')[0];
        const encontrado = data.find((item: any) => item.date === hoje);
        setRemarketingDoDia(encontrado || null);
      } catch (err) {
  ...
  const handleMostrarRemarketing = () => {
    setMostrarRemarketing(true);
  };
  ...
  return (
    <div>
      ...
      <button
  onClick={handleMostrarRemarketing}
  type="button"
  className="mb-4 bg-blue-100 text-blue-700 px-4 py-2 rounded-md hover:bg-blue-200 transition"
>
  📩 Ver Remarketing de Hoje
</button>
{/* Exibir o conteúdo do remarketing ou a mensagem se não houver */}
{mostrarRemarketing && remarketingDoDia ? (
  <div className="mt-4 p-4 bg-white border rounded-lg shadow">
    <p className="text-sm font-medium mb-2">{remarketingDoDia.text}</p>
    <p className="text-xs text-gray-500 mb-1">Data: {remarketingDoDia.date}</p>
    {remarketingDoDia.image && (
      <img src={remarketingDoDia.image} alt="Imagem do remarketing" className="mt-2 w-full max-w-xs rounded" />
    )}
  </div>
) : mostrarRemarketing && !remarketingDoDia ? (
  <p className="text-sm text-gray-500 mt-4">Nenhum conteúdo disponível para hoje.</p>
) : null}
