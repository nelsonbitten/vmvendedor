...
const MessageList: React.FC<MessageListProps> = ({ messages, isTyping, isDarkMode, onSendMessage }) => {
  ...
  const showSimButton = (message: ChatMessage) => {
    return message.sender === 'ai' && 
           (message.text.includes('Gostaria que eu enviasse o remarketing do dia') ||
            message.text.includes('Gostaria de ver o segundo remarketing do dia')) &&
           !message.image;
  };
  ...
  return (
    <div className={`absolute inset-0 overflow-y-auto p-4 space-y-4 overscroll-none ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
      {messages.length === 0 && (
        <div className={`text-center ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} mt-8`}>
          <Bot className={`w-12 h-12 mx-auto mb-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
          <p className={`text-lg font-medium mb-6 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
...
>
  <MessageSquare className={`w-5 h-5 mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} />
  <div>
    ...
    <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      {messages.map((message) => (
        ...
        >
          ...
          <div className="relative group max-w-[80%]">
            ...
            >
              <p className="text-[15px] leading-relaxed whitespace-pre-wrap pr-8">{message.text}</p>
              ...
              <span className={`text-[11px] sm:text-xs mt-2 block ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      {isTyping && (
        <div className="flex items-start gap-3">
          ...
          <div className={`rounded-lg ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-[#f7f7f8] border border-gray-200'}`}>
